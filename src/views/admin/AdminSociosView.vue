...
<template>
  <AdminShell active="socios" title="Gestión de Empresas" subtitle="Listado general y altas del padrón">
    <div class="company-actions"><button class="company-button company-button--outline" type="button"><Download :size="16" /> Descargar Excel</button><RouterLink class="company-button company-button--primary" to="/admin/socios/nueva"><Plus :size="16" /> Nueva Empresa</RouterLink></div>
    <section class="company-search-card"><label class="company-search"><Search :size="17" /><input placeholder="Buscar por razón social, RUT, BPS o rubro..."></label><span>6 de 10 empresas listadas</span></section>
    <section class="company-table-card"><div class="company-table-wrap"><table class="company-table"><thead><tr><th>Empresa<br><small>BPS</small></th><th>Rubro</th><th>Cuota</th><th>Estado</th><th>Alta</th><th>Activo</th><th>Acciones</th></tr></thead><tbody><tr v-for="company in companies" :key="company[0]"><td class="company-table__name"><strong>{{ company[0] }}</strong><small>{{ company[1] }}</small></td><td>{{ company[3] }}</td><td class="company-table__fee">$3.500</td><td><span class="cash-status" :class="`cash-status--${company[4] === 'Al Día' ? 'green' : company[4] === 'Pendiente' ? 'orange' : 'red'}`"><i />{{ company[4] }}</span></td><td>{{ company[5] }}</td><td><button class="company-toggle" :class="{ 'company-toggle--on': company[6] }" type="button" :aria-label="company[6] ? 'Pasar empresa a inactivo' : 'Quitar inactividad de la empresa'" @click="toggleActive(company)"><i /></button></td><td><button class="company-edit" type="button"><Pencil :size="12" /> Editar</button></td></tr></tbody></table></div></section>
  </AdminShell>
</template>
